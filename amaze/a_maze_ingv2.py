...
# --- Import Student A's module ---
# Once Student A delivers mazegen/, replace this stub with:
#   from mazegen.generator import MazeGenerator
#
# For now, we use a placeholder so YOUR code can already run.
try:
    from mazegen.generator import MazeGenerator  # type: ignore[import]
except ImportError:
    # Stub so the file runs even before Student A delivers her module.
    # Remove this block once mazegen is available.
    class MazeGenerator:  # type: ignore[no-redef]
...
def run(config: MazeConfig) -> None:
    ...
    # Create the generator from Student A's class
    gen = MazeGenerator(
        width=config.width,
    ...
    while True:
        ...
        if choice == "1":
            ...
            gen_new = MazeGenerator(
                ...
                exit_=config.exit,
                perfect=config.perfect,
                seed=None,  # no seed → new random maze each time
            )
            gen_new.generate()
            gen = gen_new
            show_path = False
